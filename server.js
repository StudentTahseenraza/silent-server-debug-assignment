...
app.use(cookieParser()); // FIX: Add cookie parser middleware
...
app.post("/auth/login", (req, res) => {
  try {
    ...
    // Store OTP - FIX: Store as string for consistent comparison
    otpStore[loginSessionId] = otp.toString();

    console.log(`[OTP] Session ${loginSessionId} generated: ${otp}`); // FIX: Actually log the OTP
...
app.post("/auth/verify-otp", (req, res) => {
  try {
    ...
    // FIX: Compare strings instead of number vs string
    if (otp.toString() !== otpStore[loginSessionId]) {
...
app.post("/auth/token", (req, res) => {
  try {
    // ✅ FIX: Get session from cookie instead of Authorization header
